...
Pour chaque graphique ci-dessous, proposez du code dans le chunk portant le nom `chartXX` où `XX` est le numéro du graphique de 01 à 20. Ensuite **Knittez** le document pour obtenir votre score et générer le fichier de résultats. Pour obtenir un score parfait (= 1), il faut que vous reproduisiez le graphique *parfaitement à l'identique* ! Ne changez pas le nom de ce fichier, ne le déplacez pas et n'en créez pas d'autres : tout doit être dans `docs/challenge_graphiques.Rmd` !

**Notez ceci :** - Si les couleurs sont forcées hors palettes, nous utilisons des couleurs pures comme `"gray"`, `"red"`, `"green"`, `"blue"`, ... - Si une transparence est appliquée, sa valeur est toujours de **0.5**. - **N'éditez rien dans ce document en dehors du nom d'auteur et de l'intérieur des chunks `chartXX` !!!**
...
# des 104 phalanger renards (une espèce d'opposum), capturés en sept sites
...
Votre score total est de **`r res <- score_all_charts(); round(sum(res), 1)`/20**. Si vous soumettez votre réponse (fichier results/`r  attr(res, "file")`), faites un commit avec comme message le numéro **`r attr(res, "id")`**.
